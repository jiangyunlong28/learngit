function remote_up_icon() {
    var _text = remote["dir"] == "oss" ? "集群": "七牛";
    $(".progress-container").hide();
    $(".redirect-tips").show().text("正在同步到" + _text + "云...");
    var _url = in_path + "source/pack/" + remote["dir"] + "/upload_progress.php?ac=icon&id=" + in_id + "&uid=" + in_uid + "&upw=" + in_upw;
    if (remote["version"] >= 0) {
...
function remote_up_app() {
    var _text = remote["dir"] == "oss" ? "集群": "七牛";
    $(".redirect-tips").text("正在同步到" + _text + "云...");
    var _url = in_path + "source/pack/" + remote["dir"] + "/upload_progress.php?ac=app&time=" + in_time + "&uid=" + in_uid + "&upw=" + in_upw;
    if (remote["version"] >= 0) {
        $("body").append('<iframe width="0" height="0" allowtransparency="true" scrolling="no" border="0" frameborder="0" src="' + _url + '"></iframe>');
        return;
    }
...
function edit_app() {
    ...
    if ($("#in_pwd").val() == "") {
        ...
        var reg = /^[0-9]+.?[0-9]*$/;
        if (reg.test($("#in_pwd").val()) && $("#in_pwd").val().length > 0 && $("#in_pwd").val().length <= 6) {
            xhr.open("GET", in_path + "source/index/ajax_profile.php?ac=edit&name=" + escape($("#in_name").val()) + "&link=" + $("#in_link").val() + "&pwd=" + $("#in_pwd").val() + "&id=" + in_id, true);
            xhr.onreadystatechange = function() {
                if (xhr.readyState == 4) {
                    if (xhr.status == 200) {
                        if (xhr.responseText == -1) {
            ...
            if (xhr.readyState == 4) {
                if (xhr.status == 200) {
                    if (xhr.responseText == -1) {
                        ...
                        layer.msg("您不能删除签名期内的应用，请使用更新应用功能！", 3, 8);
                    } else if (xhr.responseText == 1) {
        ...
        if (xhr.readyState == 4) {
            if (xhr.status == 200) {
                if (xhr.responseText == -1) {
                    ...
                    $(".redirect-tips").text("系统授权异常，请稍后重试！");
