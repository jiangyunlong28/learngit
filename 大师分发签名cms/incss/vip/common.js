...
$(function() {
    ...
    function checkSize(filesize) {
        ...
        if (filesize + userJson.uploadedSize > userJson.allowSize) {
            bError = true;
            alert("您的使用空间不足！【购买vip不限制容量】");
            return
        }
        if (filesize > userJson.singleAllow) {
            bError = true;
            alert("超出文件上传体积！【购买vip不限制大小】");
            return
        } else {
            return true
        }
                ...
                var uploader = Qiniu.uploader({
                    ...
                    init: {
                        ...
                        'FileUploaded': function (up, file, info) {
                            ...
                            //     $(".qq-drop-processing span").eq(0).html("请用谷歌与360浏览器……请稍后……")
                            // } else {
                            //     storeInfo()
                            // }
                        },
        ...
        var uploader = new plupload.Uploader({
            ...
            init: {
                ...
                FileUploaded: function(up, file, info) {
                    ...
                    if(userJson.type=='2'){
                        mmerror = JSON.parse(jsonData.response).error;
                        if(mmerror){
                            alert(mmerror);
                            window.location.href = "/Index/index.html";
                            return false;
                        }else{
                            //alert(bdupload+"上传成功！"+sDownLink+'==='+iLocalId+'==='+sext+'==='+iAppSize);
                            uploadftp(sDownLink,iAppSize,iLocalId);
                        }
                    }else{
    ...
    $.ajax({
        ...
        success: function(json) {
           ...
           //window.location.href = "/a/" + json.id
            alert("上传成功请添加图片和基本信息");
           window.location.href = "/index/modify/id/" + json.id+".html";
        },
        error: function(data) {
...
function uploadftp(name,iAppSize,iLocalId) {

    var uid = $('#loginuid').val();
    $.ajax({
        type: 'POST',
        //url: '/Index/uploadftp.html',
        url: '/dafuapk.php',
        dataType: "json",
        data: {
            "name": name,
