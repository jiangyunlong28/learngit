...
function showPay(rmb,mytype){
  	_mytype=mytype;
    _rmb = rmb;
	 $(".dialog-mask").show();
     $("#buy-confirm-all").show();
}

function pay(zftype) {
  _zftype=zftype;
  $("#buy-confirm-all").hide();
  if (in_pay == 2 || _zftype == 2) {
        $(".dialog-mask").show();
        $("#buy-confirm-zfb").show();
        $(".btn-yellow").attr("onclick", "window.open('" + in_path + "source/pack/alipay/pay.php?rmb=" + _rmb + "&mytype=" + _mytype + "')");
    } else {
        var xhr = new XMLHttpRequest();
        xhr.open("GET", in_path + "source/pack/weixin/pay.php?rmb=" + _rmb + "&mytype=" + _mytype, true);
        xhr.onreadystatechange = function() {
            if (xhr.readyState == 4) {
                if (xhr.status == 200) {
                    if (xhr.responseText == -1) {
	...
	xhr.onreadystatechange = function() {
		if(xhr.readyState == 4) {
			if(xhr.status == 200) {
				...
				} else if(xhr.responseText == "return_0") {
					document.getElementById("alert-warning").innerHTML = "<ul><li>邮件码为空或错误,请获取!</li></ul>";
				} else if(xhr.responseText == "return_3") {
...
function send_mail_reg() {
	...
	document.getElementById("send_btn_reg").innerHTML = "获取中...";
	xhr.open("GET", in_path + "source/index/ajax.php?ac=send_reg&mail=" + mail.value, true);
	xhr.onreadystatechange = function() {
		if(xhr.readyState == 4) {
			if(xhr.status == 200) {
				if(xhr.responseText == "return_0") {
			...
			document.getElementById("send_btn_reg").innerHTML = "重新获取";
		}
	};
	xhr.send(null);
}
